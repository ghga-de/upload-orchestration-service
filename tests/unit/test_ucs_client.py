...
from uos.adapters.outbound.http import UCSClient
...
async def test_create_file_upload_box(config: ConfigFixture, httpx_mock: HTTPXMock):
    ...
    ucs_client = UCSClient(config=config.config)
    httpx_mock.add_response(201, json=str(TEST_BOX_ID))
    box_id = await ucs_client.create_file_upload_box(storage_alias="HD01")
    ...
    with pytest.raises(UCSClient.UCSCallError):
        await ucs_client.create_file_upload_box(storage_alias="HD01")
...
async def test_lock_file_upload_box(config: ConfigFixture, httpx_mock: HTTPXMock):
    ...
    await ucs_client.lock_file_upload_box(box_id=TEST_BOX_ID)  # no error == success

    # Check off-normal status code
    httpx_mock.add_response(500, json="Some error occurred.")
    with pytest.raises(UCSClient.UCSCallError):
        await ucs_client.lock_file_upload_box(box_id=TEST_BOX_ID)
...
async def test_unlock_file_upload_box(config: ConfigFixture, httpx_mock: HTTPXMock):
    ...
    ucs_client = UCSClient(config=config.config)
    httpx_mock.add_response(204)
    await ucs_client.unlock_file_upload_box(box_id=TEST_BOX_ID)  # no error == success

    # Check off-normal status code
    httpx_mock.add_response(500, json="Some error occurred.")
    with pytest.raises(UCSClient.UCSCallError):
        await ucs_client.unlock_file_upload_box(box_id=TEST_BOX_ID)
...
async def test_get_file_upload_list(config: ConfigFixture, httpx_mock: HTTPXMock):
    ...
    file_list = await ucs_client.get_file_upload_list(box_id=TEST_BOX_ID)
    assert file_list == file_list_response

    # Check off-normal status code
    httpx_mock.add_response(500, json="Some error occurred.")
    with pytest.raises(UCSClient.UCSCallError):
        await ucs_client.get_file_upload_list(box_id=TEST_BOX_ID)
